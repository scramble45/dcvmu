module.exports = function (fileName, callback) {
  ...
  async.auto({
    gatherFileSize: function (cb) {
      var fileSizeInBytes = fs.statSync(`${__dirname}/../uploads/${fileName}.VMS`)
    ...
    gatherChecksumData: function (cb) {
      var checksumData = JSON.stringify(Buffer.from(fileName, 'utf8'))
    ...
    fs.writeFile(`${__dirname}/../uploads/${fileName}.VMI`, results.createBuffer, (err) => {
      if (err) return callback(err)
      var msg = {
        status: true,
        message: `Successfully wrote out VMI file to: uploads/${fileName}.VMI`
      }
